...
Деливередлы:
- [ ] `alembic init migrations` и интеграция с асинхронным engine.
- [ ] Первая миграция `0001_initial`:
  - `sync_run`, `sync_file`, `ip_allocation`, `asn_allocation`
    (см. `docs/03-database-schema.md`).
- [ ] `src/rir2localdb/config.py` — pydantic-settings, читает env.
- [ ] `src/rir2localdb/db/engine.py` — фабрика async engine'а и
  sessionmaker'а.
- [ ] `src/rir2localdb/db/models.py` — SQLAlchemy декларативные модели.
- [ ] `src/rir2localdb/sync/fetcher.py` — `fetch(source) -> FetchResult`.
  Включает: md5-чек, condиtional GET, потоковую запись на диск,
  retry+backoff.
- [ ] `src/rir2localdb/sync/state.py` — CRUD над `sync_file`.
- [ ] `src/rir2localdb/sync/orchestrator.py` — `run_sync(tiers)` —
  открывает `sync_run`, итерирует источники, диспатчит на
  парсер+ETL, закрывает run.
- [ ] `src/rir2localdb/parsers/delegated.py` — итератор записей.
- [ ] `src/rir2localdb/etl/delegated_etl.py` — `COPY` в staging,
  `INSERT ON CONFLICT` в основные таблицы, обновление
  `last_seen_run`.
- [ ] `src/rir2localdb/api/app.py` — FastAPI приложение, lifespan
  для async engine.
- [ ] `src/rir2localdb/api/routers/ip.py` — `GET /v1/ip/{addr}`.
- [ ] `src/rir2localdb/api/routers/asn.py` — `GET /v1/asn/{num}`.
- [ ] `src/rir2localdb/api/routers/status.py` — `/v1/status`, `/v1/healthz`.
- [ ] `src/rir2localdb/cli.py` — typer-команды: `sync`, `api`,
  `status`, `migrate`, `gc`.
- [ ] `tests/test_parsers_delegated.py` — на фрагментах от каждого RIR.
- [ ] `tests/test_api_smoke.py` — поднимает app c sqlite-like memory
  Postgres (testcontainers) и дёргает endpoints.
- [ ] CI на GitHub Actions: ruff + pytest + alembic check (Stage 1
  достаточно одного workflow).
